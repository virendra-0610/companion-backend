...
/**
 * Single-user token policy:
 * - Firestore may contain old enabled tokens from earlier PWA installs/restarts.
 * - For delivery, backend must use only the latest active token.
 * - Old enabled duplicates can be disabled with /api/cleanup-tokens.
 */
export async function getNotificationTokens(options = {}) {
  ...
  const latest = pickLatestToken(active);
...
export async function getAllNotificationTokens({ includeDisabled = false } = {}) {
  ...
  tokens.sort((a, b) => {
    const delta = (b.sortTime || 0) - (a.sortTime || 0);
...
export async function disableOlderActiveTokens({ keepDocId = null, reason = "backend_token_cleanup" } = {}) {
  ...
  const keep = keepDocId
    ? allActive.find((item) => item.docId === keepDocId) || pickLatestToken(allActive)
    : pickLatestToken(allActive);

  if (!keep) {
    return { kept: null, disabled: 0, totalActiveBefore: 0, results: [] };
  }
  ...
  allActive.forEach((item) => {
    if (item.docId === keep.docId) return;
    ...
    batch.set(
      ref,
      {
        ...
        replacedByTokenDocId: keep.docId,
        updatedAt: new Date()
      },
      { merge: true }
    );
  ...
  return {
    kept: {
      docId: keep.docId,
      tokenTail: keep.token.slice(-8),
      location: getLocationLabel(keep.data)
    },
