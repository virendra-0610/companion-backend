import { getFirestore } from "./firebaseAdmin.js";
import { getNotificationTokens, sendPushToToken, removeBadToken, writeNotificationHistory, safeHistoryKey, TOKEN_POLICY } from "./push.js";

const TASK_COLLECTION = "todo_tasks";
const TOKEN_COLLECTION = "notification_tokens";
const FIVE_MINUTES_MS = 5 * 60 * 1000;
const TEN_MINUTES_MS = 10 * 60 * 1000;
const THIRTY_MINUTES_MS = 30 * 60 * 1000;
const ONE_DAY_MS = 24 * 60 * 60 * 1000;
const AUTO_COMPLETE_GRACE_MS = 15 * 60 * 1000;
const DEFAULT_TIME_ZONE = "Asia/Kolkata";

export async function checkTaskReminders({
  force = false,
  dryRun = false,
  cleanupExpired = false,
  limit = 200,
  taskId = "",
  timeZone = ""
} = {}) {
  const db = getFirestore();
  const now = new Date();

  const loadResult = await loadCandidateTasks(db, { limit, taskId, now, force, dryRun, cleanupExpired });
  const docs = loadResult.docs;

  const results = [];
  let sent = 0;
  let skipped = 0;
  let failed = 0;
  let autoCompleted = 0;

  for (const doc of docs) {
    try {
      const task = doc.data() || {};
      const effectiveTaskId = doc.id;
      const dueAt = toDate(task.dueAt);

      if (!dueAt) {
        skipped += 1;
        results.push({ taskId: effectiveTaskId, ok: true, skipped: true, reason: "Missing dueAt" });
        continue;
      }

      if (task.completed === true || task.deleted === true) {
        skipped += 1;
        results.push({ taskId: effectiveTaskId, ok: true, skipped: true, reason: "Completed or deleted" });
        continue;
      }

      const tokenResolution = await resolveTaskTokens(db, task);
      const effectiveTimeZone = resolveTimeZone({
        explicitTimeZone: timeZone,
        task,
        tokenData: tokenResolution.primaryTokenData
      });

      const reminder = force
        ? buildForcedReminder(task, dueAt, effectiveTimeZone)
        : chooseReminder(task, dueAt, now);

      if (!reminder) {
        const expired = dueAt.getTime() < now.getTime() - AUTO_COMPLETE_GRACE_MS;

        if (expired && !dryRun && !force) {
          await markTaskAutoCompleted(doc.ref);
          autoCompleted += 1;
          results.push({
            taskId: effectiveTaskId,
            ok: true,
            autoCompleted: true,
            title: safeTitle(task.title),
            dueAtUtc: dueAt.toISOString(),
            dueAtLocal: formatDateTime(dueAt, effectiveTimeZone),
            timeZone: effectiveTimeZone,
            reason: "Expired task auto-completed by backend"
          });
          continue;
        }

        skipped += 1;
        results.push({
          taskId: effectiveTaskId,
          ok: true,
          skipped: true,
          title: safeTitle(task.title),
          dueAtUtc: dueAt.toISOString(),
          dueAtLocal: formatDateTime(dueAt, effectiveTimeZone),
          timeZone: effectiveTimeZone,
          reason: expired ? "Expired task would be auto-completed" : "No reminder due"
        });
        continue;
      }

      if (!tokenResolution.tokens.length) {
        skipped += 1;
        await markTaskError(doc.ref, `Missing active FCM token for task ${effectiveTaskId}`);
        results.push({ taskId: effectiveTaskId, ok: false, skipped: true, reason: "Missing active FCM token" });
        continue;
      }

      const title = reminder.title;
      const body = reminder.body;

      if (dryRun) {
        skipped += 1;
        results.push({
          taskId: effectiveTaskId,
          ok: true,
          dryRun: true,
          reminderType: reminder.type,
          title,
          body,
          dueAtUtc: dueAt.toISOString(),
          dueAtLocal: formatDateTime(dueAt, effectiveTimeZone),
          timeZone: effectiveTimeZone,
          tokenSource: tokenResolution.source,
          tokenCount: tokenResolution.tokens.length,
          tokens: tokenResolution.tokens.map((item) => ({
            tokenDocId: item.tokenDocId || null,
            tokenTail: item.token ? item.token.slice(-8) : null
          }))
        });
        continue;
      }

      const deliveryResults = [];
      let delivered = 0;
      let deliveryFailed = 0;

      for (const target of tokenResolution.tokens) {
        try {
          const messageId = await sendPushToToken({
            token: target.token,
            title,
            body,
            data: {
              type: "task_reminder",
              reminderType: reminder.type,
              taskId: effectiveTaskId,
              dueAt: dueAt.toISOString(),
              dueAtLocal: formatDateTime(dueAt, effectiveTimeZone),
              timeZone: effectiveTimeZone,
              force: force ? "true" : "false"
            }
          });

          delivered += 1;
          deliveryResults.push({ tokenDocId: target.tokenDocId || null, ok: true, messageId });
        } catch (deliveryError) {
          const msg = deliveryError.message || String(deliveryError);
          deliveryFailed += 1;

          if (target.tokenDocId && (msg.includes("registration-token-not-registered") || msg.includes("invalid-registration-token"))) {
            await removeBadToken(target.tokenDocId);
          }

          deliveryResults.push({ tokenDocId: target.tokenDocId || null, ok: false, error: msg });
        }
      }

      if (delivered === 0) {
        failed += 1;
        await markTaskError(doc.ref, deliveryResults.find((item) => item.ok === false)?.error || "All task reminder deliveries failed");
        results.push({
          taskId: effectiveTaskId,
          ok: false,
          reminderType: reminder.type,
          reason: "All task reminder deliveries failed",
          deliveries: deliveryResults
        });
        continue;
      }

      if (!force) {
        await markReminderSent(doc.ref, reminder.type);
        await writeNotificationHistory({
          key: safeHistoryKey(`task_${effectiveTaskId}_${reminder.type}_${dueAt.toISOString()}`),
          source: "task",
          type: `task_reminder_${reminder.type}`,
          title,
          body,
          status: "sent",
          runMode: "scheduled",
          taskId: effectiveTaskId,
          tokenDocId: tokenResolution.tokens[0]?.tokenDocId || null,
          eventName: safeTitle(task.title),
          eventDate: formatDateTime(dueAt, effectiveTimeZone),
          data: {
            reminderType: reminder.type,
            dueAtUtc: dueAt.toISOString(),
            dueAtLocal: formatDateTime(dueAt, effectiveTimeZone),
            timeZone: effectiveTimeZone,
            tokenSource: tokenResolution.source,
            sentCount: delivered,
            failedCount: deliveryFailed,
            tokenDocIds: tokenResolution.tokens.map((item) => item.tokenDocId).filter(Boolean),
            tokenPolicy: TOKEN_POLICY
          }
        });
      } else {
        await markForcedTest(doc.ref, reminder.type);
      }

      sent += delivered;
      failed += deliveryFailed;
      results.push({
        taskId: effectiveTaskId,
        ok: true,
        reminderType: reminder.type,
        delivered,
        deliveryFailed,
        dueAtUtc: dueAt.toISOString(),
        dueAtLocal: formatDateTime(dueAt, effectiveTimeZone),
        timeZone: effectiveTimeZone,
        tokenSource: tokenResolution.source,
        tokenCount: tokenResolution.tokens.length,
        deliveries: deliveryResults
      });
    } catch (itemError) {
      failed += 1;
      results.push({
        taskId: doc.id,
        ok: false,
        error: itemError.message || String(itemError)
      });

      try {
        await markTaskError(doc.ref, itemError.message || String(itemError));
      } catch (_) {}
    }
  }

  return {
    nowUtc: now.toISOString(),
    nowLocal: formatDateTime(now, timeZone || process.env.DEFAULT_TIME_ZONE || DEFAULT_TIME_ZONE),
    requestedTaskId: taskId || null,
    cleanupExpired,
    loadMode: loadResult.mode,
    scanned: docs.length,
    sent,
    skipped,
    failed,
    autoCompleted,
    results
  };
}

async function loadCandidateTasks(db, { limit, taskId, now, force, dryRun, cleanupExpired }) {
  const cleanTaskId = String(taskId || "").trim();

  if (cleanTaskId) {
    const directSnap = await db.collection(TASK_COLLECTION).doc(cleanTaskId).get();
    if (directSnap.exists) return { docs: [directSnap], mode: "direct_doc" };

    const byIdSnap = await db.collection(TASK_COLLECTION).where("id", "==", cleanTaskId).limit(1).get();
    return { docs: byIdSnap.docs, mode: "where_id" };
  }

  try {
    const snap = await db
      .collection(TASK_COLLECTION)
      .where("completed", "==", false)
      .where("deleted", "==", false)
      .limit(limit)
      .get();

    return { docs: sortAndTrimTasks(snap.docs, { limit, now, force, dryRun, cleanupExpired }), mode: cleanupExpired ? "filtered_query_cleanup_expired" : "filtered_query" };
  } catch (error) {
    // Avoid scheduler outage if Firestore requires a composite index or a legacy doc breaks the query.
    const snap = await db.collection(TASK_COLLECTION).get();
    return {
      docs: sortAndTrimTasks(snap.docs, { limit, now, force, dryRun, cleanupExpired }),
      mode: `fallback_full_scan_after_query_error:${String(error.message || error).slice(0, 120)}`
    };
  }
}

function sortAndTrimTasks(docs, { limit, now, force, dryRun, cleanupExpired }) {
  const nowMs = now.getTime();
  const lookBehindMs = cleanupExpired ? 60 * ONE_DAY_MS : (force || dryRun ? 14 * ONE_DAY_MS : 6 * 60 * 60 * 1000);
  const lookAheadMs = force || dryRun ? 14 * ONE_DAY_MS : ONE_DAY_MS + THIRTY_MINUTES_MS;

  return docs
    .filter((doc) => {
      const task = doc.data() || {};
      if (task.completed === true || task.deleted === true) return false;
      const dueAt = toDate(task.dueAt);
      if (!dueAt) return true;
      const dueMs = dueAt.getTime();
      return dueMs >= nowMs - lookBehindMs && dueMs <= nowMs + lookAheadMs;
    })
    .sort((a, b) => {
      const ad = toDate((a.data() || {}).dueAt)?.getTime() || Number.MAX_SAFE_INTEGER;
      const bd = toDate((b.data() || {}).dueAt)?.getTime() || Number.MAX_SAFE_INTEGER;
      return ad - bd;
    })
    .slice(0, limit);
}

function chooseReminder(task, dueAt, now) {
  const dueMs = dueAt.getTime();
  const nowMs = now.getTime();
  const untilDueMs = dueMs - nowMs;

  if (!task.reminderDueSent && dueMs <= nowMs && dueMs >= nowMs - TEN_MINUTES_MS) {
    return {
      type: "due",
      flag: "reminderDueSent",
      title: "Task due now",
      body: `${safeTitle(task.title)} is due now.`
    };
  }

  if (!task.reminder30MinSent && untilDueMs > 0 && untilDueMs <= THIRTY_MINUTES_MS) {
    return {
      type: "30min",
      flag: "reminder30MinSent",
      title: "Task reminder",
      body: `${safeTitle(task.title)} is due in about ${formatMinutes(untilDueMs)}.`
    };
  }

  const oneDayReminderAt = dueMs - ONE_DAY_MS;
  const oneDayWindowStart = nowMs - FIVE_MINUTES_MS;
  const oneDayWindowEnd = nowMs + FIVE_MINUTES_MS;

  if (
    !task.reminder1DaySent &&
    oneDayReminderAt >= oneDayWindowStart &&
    oneDayReminderAt <= oneDayWindowEnd &&
    untilDueMs > THIRTY_MINUTES_MS
  ) {
    return {
      type: "1day",
      flag: "reminder1DaySent",
      title: "Task reminder for tomorrow",
      body: `${safeTitle(task.title)} is due tomorrow.`
    };
  }

  return null;
}

function buildForcedReminder(task, dueAt, timeZone) {
  return {
    type: "forced_task_test",
    flag: null,
    title: "Task reminder test",
    body: `${safeTitle(task.title)} is scheduled for ${formatDateTime(dueAt, timeZone)}.`
  };
}

async function resolveTaskTokens(db, task) {
  const activeTokens = await getNotificationTokens();
  const deliveryTargets = activeTokens
    .filter((item) => {
      const data = item.data || {};
      return getBooleanSetting(data, [
        "taskRemindersEnabled",
        "taskReminders",
        "enableTaskReminders"
      ], true);
    })
    .map((item) => ({
      token: item.token,
      tokenDocId: item.docId,
      tokenData: item.data || null,
      source: "notification_tokens_multi_device"
    }));

  if (deliveryTargets.length > 0) {
    return {
      tokens: deliveryTargets,
      primaryTokenData: deliveryTargets[0].tokenData,
      source: "notification_tokens_multi_device"
    };
  }

  const tokenDocId = typeof task.tokenDocId === "string" ? task.tokenDocId.trim() : "";

  if (tokenDocId) {
    const tokenSnap = await db.collection(TOKEN_COLLECTION).doc(tokenDocId).get();
    if (tokenSnap.exists) {
      const data = tokenSnap.data() || {};
      const token = firstString(data.token, data.fcmToken, data.notificationToken);
      const enabled = data.enabled !== false;
      const taskRemindersEnabled = getBooleanSetting(data, [
        "taskRemindersEnabled",
        "taskReminders",
        "enableTaskReminders"
      ], true);

      if (token && enabled && taskRemindersEnabled) {
        return {
          tokens: [{
            token,
            tokenDocId,
            tokenData: data,
            source: "notification_tokens_matching_task"
          }],
          primaryTokenData: data,
          source: "notification_tokens_matching_task"
        };
      }
    }
  }

  const legacyToken = firstString(task.token, task.fcmToken, task.notificationToken);
  if (legacyToken) {
    return {
      tokens: [{
        token: legacyToken,
        tokenDocId: tokenDocId || null,
        tokenData: null,
        source: "task_document_legacy_fallback"
      }],
      primaryTokenData: null,
      source: "task_document_legacy_fallback"
    };
  }

  return {
    tokens: [],
    primaryTokenData: null,
    source: "missing_token"
  };
}

function extractSortTime(value) {
  if (!value) return 0;
  if (typeof value.toDate === "function") return value.toDate().getTime();
  if (value instanceof Date) return value.getTime();
  if (typeof value === "string") {
    const parsed = new Date(value);
    return Number.isFinite(parsed.getTime()) ? parsed.getTime() : 0;
  }
  return 0;
}

async function markReminderSent(ref, type) {
  const update = {
    lastReminderSentAt: new Date(),
    lastReminderType: type,
    lastReminderError: null,
    lastReminderErrorAt: null,
    updatedAt: new Date()
  };

  if (type === "1day") update.reminder1DaySent = true;
  if (type === "30min") update.reminder30MinSent = true;
  if (type === "due") update.reminderDueSent = true;

  await ref.set(update, { merge: true });
}

async function markForcedTest(ref, type) {
  await ref.set(
    {
      lastReminderTestSentAt: new Date(),
      lastReminderTestType: type,
      updatedAt: new Date()
    },
    { merge: true }
  );
}

async function markTaskAutoCompleted(ref) {
  await ref.set(
    {
      completed: true,
      completedAt: new Date(),
      autoCompleted: true,
      completionReason: "expired_by_backend",
      updatedAt: new Date()
    },
    { merge: true }
  );
}

async function markTaskError(ref, message) {
  await ref.set(
    {
      lastReminderError: String(message).slice(0, 500),
      lastReminderErrorAt: new Date(),
      updatedAt: new Date()
    },
    { merge: true }
  );
}

function toDate(value) {
  if (!value) return null;
  if (typeof value.toDate === "function") return value.toDate();
  if (value instanceof Date) return value;
  if (typeof value === "string") {
    const parsed = new Date(value);
    return Number.isFinite(parsed.getTime()) ? parsed : null;
  }
  return null;
}

function firstString(...values) {
  for (const value of values) {
    if (typeof value === "string" && value.trim()) return value.trim();
  }
  return null;
}

function getBooleanSetting(settings, keys, defaultValue) {
  for (const key of keys) {
    if (typeof settings[key] === "boolean") return settings[key];
  }
  return defaultValue;
}

function safeTitle(title) {
  const trimmed = String(title || "Task").trim();
  return trimmed || "Task";
}

function formatMinutes(ms) {
  const minutes = Math.max(1, Math.round(ms / 60000));
  if (minutes === 1) return "1 minute";
  return `${minutes} minutes`;
}

function resolveTimeZone({ explicitTimeZone = "", task = {}, tokenData = null }) {
  const selectedLocation = tokenData?.selectedLocation || tokenData?.currentLocation || tokenData?.location || null;
  const city = firstString(
    selectedLocation?.city,
    selectedLocation?.name,
    selectedLocation?.label,
    tokenData?.city,
    task.city
  );

  const candidate = firstString(
    explicitTimeZone,
    task.timeZone,
    task.timezone,
    task.tz,
    selectedLocation?.timeZone,
    selectedLocation?.timezone,
    selectedLocation?.tz,
    tokenData?.timeZone,
    tokenData?.timezone,
    process.env.DEFAULT_TIME_ZONE
  );

  if (isValidTimeZone(candidate)) return candidate;

  const cityTimeZone = inferTimeZoneFromCity(city);
  if (cityTimeZone) return cityTimeZone;

  return DEFAULT_TIME_ZONE;
}

function inferTimeZoneFromCity(city) {
  const c = String(city || "").trim().toLowerCase();
  if (!c) return null;

  if (c.includes("ahmedabad") || c.includes("gandhinagar") || c.includes("gujarat") || c.includes("india")) {
    return "Asia/Kolkata";
  }
  if (c.includes("riga") || c.includes("latvia")) return "Europe/Riga";

  return null;
}

function isValidTimeZone(timeZone) {
  if (!timeZone || typeof timeZone !== "string") return false;

  try {
    new Intl.DateTimeFormat("en-US", { timeZone }).format(new Date());
    return true;
  } catch (_) {
    return false;
  }
}

function formatDateTime(date, timeZone) {
  const tz = isValidTimeZone(timeZone) ? timeZone : DEFAULT_TIME_ZONE;

  const formatted = new Intl.DateTimeFormat("en-GB", {
    timeZone: tz,
    day: "2-digit",
    month: "short",
    year: "numeric",
    hour: "2-digit",
    minute: "2-digit",
    hour12: false
  }).format(date);

  return `${formatted} (${tz})`;
}
