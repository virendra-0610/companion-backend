...
export async function checkTaskReminders({
  ...
  const loadResult = await loadCandidateTasks(db, { limit, taskId, now, force, dryRun });
...
async function loadCandidateTasks(db, { limit, taskId, now, force, dryRun }) {
  ...
  try {
    ...
    return { docs: sortAndTrimTasks(snap.docs, { limit, now, force, dryRun }), mode: "filtered_query" };
  } catch (error) {
    ...
    return {
      docs: sortAndTrimTasks(snap.docs, { limit, now, force, dryRun }),
      mode: `fallback_full_scan_after_query_error:${String(error.message || error).slice(0, 120)}`
    };
  }
}

function sortAndTrimTasks(docs, { limit, now, force, dryRun }) {
  const nowMs = now.getTime();
  const lookBehindMs = force || dryRun ? 14 * ONE_DAY_MS : 6 * 60 * 60 * 1000;
